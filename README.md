...
- **데이터 수집**: RSS 피드 및 웹 크롤링
- **엔티티 분석**: 이벤트 유형별 키워드 매칭 (콘서트, 전시, 세미나, 스포츠 등)
- **트렌드 리포트**: DuckDB 저장 + HTML 리포트로 행사/이벤트 동향 시각화
...
- 워크플로: `.github/workflows/radar-crawler.yml`
  ...
  - 환경 변수 `RADAR_CATEGORY` 값은 `event`입니다.
...
- 설정 방법:
  ...
  3) 워크플로 파일의 `RADAR_CATEGORY`가 `event`인지 확인
...
- GitHub Pages visualization: `reports/index.html` (valid HTML); https://ai-frendly-datahub.github.io/EventRadar/.
