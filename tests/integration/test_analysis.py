...
def test_entity_extraction_integration(
    ...
    assert "music_event" in article_1.matched_entities
    assert any(kw in article_1.matched_entities["music_event"] for kw in ["뮤직", "페스티벌"])

    article_2 = analyzed[1]
    assert "art_exhibition" in article_2.matched_entities
    assert "전시" in article_2.matched_entities["art_exhibition"]

    article_3 = analyzed[2]
    assert "music_event" in article_3.matched_entities
    assert "콘서트" in article_3.matched_entities["music_event"]

    article_4 = analyzed[3]
    assert "film_festival" in article_4.matched_entities
    assert "영화제" in article_4.matched_entities["film_festival"]

    article_5 = analyzed[4]
    assert "sports_event" in article_5.matched_entities
    assert "마라톤" in article_5.matched_entities["sports_event"]
