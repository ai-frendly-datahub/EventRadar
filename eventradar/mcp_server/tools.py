...
def handle_event_impact(*, db_path: Path, days: int = 30, limit: int = 10) -> str:
    """Analyze event impact by counting entity mentions over recent period."""
    cutoff = datetime.now() - timedelta(days=days)
    conn = duckdb.connect(str(db_path), read_only=True)
    try:
        rows = conn.execute(
            """
            SELECT entities_json, collected_at
            FROM articles
            WHERE collected_at >= ?
            """,
            [cutoff],
    ...
    for entities_json, _ in rows:
        ...
        for entity_name, matched in entities.items():
            if not isinstance(matched, list):
                continue
            entity_counts[entity_name] += len(matched)
            article_counts[entity_name] += 1

    if not entity_counts:
        return "No event impact data available for the recent period."

    lines = [f"Event impact analysis (last {days} days):"]
    for entity_name, mention_count in entity_counts.most_common(limit):
        articles = article_counts[entity_name]
        lines.append(f"- {entity_name}: {mention_count} mentions in {articles} article(s)")
    return "\n".join(lines)
